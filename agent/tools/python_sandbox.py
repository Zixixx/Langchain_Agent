    ...
    if isinstance(path_node, ast.Constant) and isinstance(path_node.value, str):
        if not _is_output_path_literal(path_node.value):
            raise SandboxPolicyError(
                f"Generated file writes must target the output directory "
                f"(output/ or output\\): {method_name}({path_node.value!r})"
            )
        return
    if isinstance(path_node, ast.Constant) and path_node.value is None:
        return
    raise SandboxPolicyError(
        f"Generated file writes must use a literal output path for static safety: {method_name}(...)"
    )
