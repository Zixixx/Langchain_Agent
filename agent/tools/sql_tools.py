...
"""SQL 工具：只暴露业务表数据库的 schema 和只读查询能力。"""
...
def build_sql_tools(settings: Settings):
    ...
    def db() -> LocalDatabase:
        """连接 db.sqlite3，避免 Agent 直接访问记忆、依赖和 RAG 元数据表。"""
        return LocalDatabase(settings.table_db_path)

    @tool
    def sql_database_schema() -> str:
        """Return SQLite database table names, quoted SQL table names, and column names for Text-to-SQL."""
        try:
    ...
    def sql_readonly_query(sql: str, limit: int = 100) -> str:
        """Run a read-only SQLite SELECT/WITH query. Use sql_database_schema and quote table names exactly."""
        try:
