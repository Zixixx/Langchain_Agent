...
3. C++ high-performance algorithms for sorting, sum, average, min and max.
...
- For numerical computation, prefer C++ tools for large arrays, then Python for grouping or reports.
...
- All generated output files must be written under output/. Read user-provided input files or data/ when needed, but write result CSV files, plots, reports, and generated artifacts only to output/.
...
def build_tools(settings: Settings, session_id: str):
    ...
    return [
        *build_file_tools(settings.workspace),
        *build_python_tools(
            settings.workspace,
