...
"""RAG 命令处理，负责导入文档、列出 source、删除 source 和相似度搜索。"""
...
def _knowledge_base():
    """懒加载并复用 KnowledgeBase，避免主菜单启动时或重复命令时反复加载模型。"""
    from agent.rag.knowledge_base import get_knowledge_base

    return get_knowledge_base(get_settings())


def add(path: Path, if_exists: str = "replace") -> None:
    """实现 rag-add 命令，导入文档并输出 chunk 统计。"""
    try:
        results = import_rag_path(_knowledge_base(), path, if_exists=if_exists)
    except ValueError as exc:
        raise typer.BadParameter(str(exc)) from exc
    if not results:
        typer.echo(f"No supported text files found: {path}")
        return
